...
**IMPORTANT: Run these two commands IN PARALLEL (at the same time) to avoid delays. They are independent of each other.**

1. Get the PR metadata (title, description, comments, and changed file list) — all in a **single** call:

   ```bash
   gh pr view <PR-number> --json title,body,comments,files,author
   ```

2. Get the full diff of the PR (run this **at the same time** as the command above):
   ```bash
...
1. From the output of Step 1, identify which files were modified.
...
2. Look for:
   - **Architecture Pattern compliance**: You MUST read `docs/PATTERN.md` and ensure the PR strictly follows the established conventions.
   - Code quality issues
   - Potential bugs
...
1. If the user opted for a drafted comment, provide a well-structured comment they can copy:
...
2. Execute the decision based on their choice.
...
# 1. Get all PR metadata in a single call
gh pr view 3627 --json title,body,comments,files,author

# 2. Get the full diff (at the same time)
gh pr diff 3627
```
...
## Step 4: Ask for User Confirmation & Comment Preference
...
Assuming the user chose "1" (Approve and draft a comment for me):
...
```bash
# Executing the decision using gh cli
